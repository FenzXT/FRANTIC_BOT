...
function getTicketConfig(guildId) {
  ...
  return configs[guildId] || { supportRole: "", category: "" };
}
function setTicketConfig(guildId, data) {
  const configs = loadTicketConfigs();
  configs[guildId] = { ...getTicketConfig(guildId), ...data };
  saveTicketConfigs(configs);
}
function resetTicketConfig(guildId) {
  const configs = loadTicketConfigs();
  configs[guildId] = { supportRole: "", category: "" };
  saveTicketConfigs(configs);
}
...
client.on('messageCreate', async (message) => {
  ...
  if (message.content.startsWith('!setticketrole') && message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
    ...
    setTicketConfig(message.guild.id, { supportRole: role.id });
  ...
  if (message.content.startsWith('!setticketcategory') && message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
    ...
    setTicketConfig(message.guild.id, { category: categoryId });
